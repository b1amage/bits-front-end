...
const Button = ({
  ...
  const buttonHoverAnimationStyles = `hover:-translate-y-2 hover:shadow-2xl ${
    primary ? "hover:bg-opacity-80" : "hover:bg-primary-100 hover:text-white"
  }`;

  const buttonStyles = `transition-all duration-300 text-center px-4 py-3 font-semibold ${
    primary
      ? "bg-primary-100 text-white"
      : "bg-transparent text-primary-100 border-2 border-primary-100"
  } ${
    fluid ? "w-full" : "inline-flex min-w-[200px] justify-center items-center"
  } ${
    isRound
