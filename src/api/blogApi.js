...
const blogApi = {
  ...
  async getUserBlogs(values, nextCursor = "") {
    try {
      const url = `/blog/user?userId=${values.userId}&&category=${values.currentCategory}&&title=${values.currentSearch}&&next_cursor=${nextCursor}`;
      const response = await axiosClient.get(url);
  ...
  async deleteComment(id, navigate) {
  ...
  async createBlog(values, navigate, setErr) {
    ...
    try {
      ...
      localStorage.setItem(
        "message",
        "Congratulations! your blog has been published. Now your blog can be seen by many people. Continue to develop your talents!"
      );
      navigate("/successful");
      return response;
    } catch (err) {
